    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className={`flex size-10 items-center justify-center rounded-lg ${severeAlerts.length > 0 ? 'bg-negative-bg' : 'bg-warning-bg'}`}>
            ...
            <Link key={alert.clientId} to={`/clients/${alert.clientId}`} className="block">
              <div className={`flex items-center justify-between rounded-lg border p-2.5 transition-colors hover:bg-muted/50 ${severe ? 'border-negative/30' : ''}`}>
                ...
                <div className="flex items-center gap-2 ml-2">
                  ...
                  {alert.scoreChange < 0 && (
                    <Badge className={`text-xs ${severe ? 'bg-negative-bg text-negative border-negative/30' : 'bg-warning-bg text-warning border-warning/30'}`}>
